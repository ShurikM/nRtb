...
export default function CampaignDetails({ campaign }: { campaign: Campaign }) {
  return (
      <div className="space-y-4">
          ...
          <div className="text-sm text-gray-600">Status: <span
              className="font-medium text-black">{campaign.status}</span></div>
              ...
              <div className="space-y-4">
                  {campaign.creatives.map((c, i) => (
                      <div key={i} className="border p-4 rounded-md bg-white shadow-sm">
                          ...
                          <p className="text-blue-500 text-sm mt-2">Click URL: <a href={c.click_url}>{c.click_url}</a>
                          </p>
                      </div>
                  ))}
              </div>
          ...
          <div className="mt-6">
              <h3 className="text-lg font-semibold mb-2">Targeting Rules</h3>
              <ul className="list-disc list-inside space-y-1 text-sm">
                  {campaign.targeting_rules.map((rule, i) => (
                      <li key={i}>
                          <strong>{rule.targeting_type}:</strong> {rule.targeting_value}
                      </li>
                  ))}
              </ul>
          </div>
